*** Settings ***
Documentation      Add Single Lesson
Resource           ../init.robot
...
*** Test Cases ***
Create Single Lesson With Not Conflict
    ...
    Wait Need Element     //strong[contains(.,"申請資料")]
    Wait Need Element     //tr[contains(.,"${lessonName}")][contains(.,"${BUILDING_NAME} ${classroomName1}")][contains(.,"${lessonTime} - A1")]

Create Single Lesson With Conflict
    Input Need Element    id=lessonName    ${lessonName}
    Input Need Element    id=lessonTime    ${lessonTime}
    Select Building
    Select Floor    1
    ...
    Click Need Element    id=btnSubmit
    Click Need Element    //button[contains(.,"確定申請")]
    Wait Until Page Does Not Contain Element    //strong[contains(.,"申請資料")]
...
TestSetup
    Click Need Element    //*[@id="mainDropdown"]/ul[2]/li/a
    Click Need Element    //*[@id="lesson_time"]/font[contains(.,"申請單次教室")]
    Wait Need Element    //strong[contains(.,"申請單次教室")]
